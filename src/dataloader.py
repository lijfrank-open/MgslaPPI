...
def load_data(dataset, split_mode, seed, skip_head=True):
    ...
    prot_seq_path = '/home/lijfrank/code/dataset/STRING_27k_148k/processed_data/protein.{}.sequences.dictionary.csv'.format(dataset)
    prot_r_edge_path = '/home/lijfrank/code/dataset/STRING_27k_148k/processed_data/protein.rball.edges.{}.npy'.format(dataset)
    prot_k_edge_path = '/home/lijfrank/code/dataset/STRING_27k_148k/processed_data/protein.knn.edges.{}.npy'.format(dataset)
    prot_node_path = '/home/lijfrank/code/dataset/STRING_27k_148k/processed_data/protein.nodes.{}.pt'.format(dataset)

    if os.path.exists("/home/lijfrank/code/dataset/STRING_27k_148k/processed_data/{}_ppi.pkl".format(dataset)):
        with open("/home/lijfrank/code/dataset/STRING_27k_148k/processed_data/{}_ppi.pkl".format(dataset), "rb") as tf:
            ppi_list = pickle.load(tf)
        with open("/home/lijfrank/code/dataset/STRING_27k_148k/processed_data/{}_ppi_label.pkl".format(dataset), "rb") as tf:
        ...
        with open("/home/lijfrank/code/dataset/STRING_27k_148k/processed_data/{}_ppi.pkl".format(dataset), "wb") as tf:
            pickle.dump(ppi_list, tf)
        with open("/home/lijfrank/code/dataset/STRING_27k_148k/processed_data/{}_ppi_label.pkl".format(dataset), "wb") as tf:
...
class ProteinDatasetDGL(torch.utils.data.Dataset):
    ...
    def __init__(self, prot_r_edge_path, prot_k_edge_path, prot_node_path,
        ...
        if os.path.exists(
            ...
            for i in range(len(prot_node)): # 第几个蛋白质
                prot_seq = []
                for j in range(prot_node[i].shape[0] - 1): # 第几个残基
                    prot_seq.append((j, j + 1))
                    prot_seq.append((j + 1, j))

                prot_g = dgl.graph(prot_seq).to(device)
...
def split_dataset(ppi_list, dataset, split_mode, seed):
    if not os.path.exists("/home/lijfrank/code/dataset/STRING_27k_148k/processed_data/{}_{}.json".format(dataset, split_mode)):
        if split_mode == 'random':
            ...
            with open("/home/lijfrank/code/dataset/STRING_27k_148k/processed_data/{}_{}.json".format(dataset, split_mode), 'w') as f:
                f.write(jsobj)
                f.close()
        ...
        with open("/home/lijfrank/code/dataset/STRING_27k_148k/processed_data/{}_{}.json".format(dataset, split_mode), 'r') as f:
